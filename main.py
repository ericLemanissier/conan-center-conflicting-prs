...
class Detector:
    ...
    def _post_message_for_lib(self, issue_number, l):
        conflicting_prs = [pr for pr in self.libs[l] if pr != issue_number]
        ...
        if _all_prs_referenced_in_body():
            print("all the conflicting prs (%s) are already referenced in #%s, skipping message" % (", ".join("#%s" % p for p in conflicting_prs), issue_number))
            return
        ...
        if comment_id:                   
            ...
            self._make_request("PATCH", f"/repos/{self.owner}/{self.repo}/issues/comments/{comment_id}",json=
            {
                "body": message
            })
        else:
